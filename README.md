...
The Gatling Kafka plugin initializes consumers **after** the request is sent, not before. This causes:
- Inaccurate response time metrics (includes consumer initialization overhead)
- Potential missed messages if replies arrive before consumer is ready
- Reported response times that don't reflect actual request-reply latency

See `GITHUB_ISSUE.md` for detailed analysis and reproduction steps.

## Pending to be done
- [ ] Tweak the CorrelationIdMatcher in order to identify if the received response is a Failed or Succeed Response message.
- [x] Generate and use property files for different environment executions 
- [ ] ~~Use of Maven profiles to store variables for each of the environments~~ Not needed as the environment variable is sent directly in the maven command
- [x] Dynamically build the protocols in order to avoid putting extra unnecessary variables in local environment (This will be needed just in case of the property files giving issues with the SSL variables)
- [x] Add Python-based reply bot for request-reply testing
- [x] Add topic monitoring utilities